...
In their analysis of the GHSI, Boyd et al. noted that one factor associated with higher GHSI scores was GDP per capita (6). This relationship can be seen from the figure two, which demonstrates that countries with higher GDPs per capita tend to have higher pandemic preparedness scores. 
...
Since no strong conclusions can be drawn from figure six alone, we fit a multivariate linear model (with variables air travel, pandemic preparedness, and cumulative COVID deaths per million) in order to observe if the correlation between pandemic preparedness and deaths would decrease when air travel is introduced as a confounding variable.

As shown below, when we controlled for air travel, the positive correlation between pandemic preparedness and cumulative COVID deaths per million decreased from 9.09 to 7.98. Moreover, the lower end of the uncertainty range decreased from 6.43 to 3.97. From these results, we can conclude that the strong positive correlation between pandemic preparedness and COVID deaths (9.09) was partially due to air travel being higher in countries with higher pandemic preparedness. 

The second confounding variable we decided to explore is age. COVID mortality rates for the older demographic have been significantly higher than the overall mortality rate, in part due to the propensity of this group having pre-existing health complications [SOURCE]. The share of the older population, therefore, has a direct impact on the cumulative COVID deaths per million. This relationship is shown in figure seven.
...
In figure eight, pandemic preparedness is plotted against cumulative deaths per million, with the share of older population(aged 65+) mapped to the color aesthetic. The lighter shades represent a higher share of people aged 65+ in a country and can be seen to increase in frequency with respect to darker shade as we move along the positive x-axis. Our theory that better-prepared countries tend to have a higher share of aging population can be seen as evident here. This is also in line with the fact that a higher GDP(a common string tying most of the countries with high pandemic preparedness as we saw in the introduction) leads to a higher life expectancy which in turn leads to an aging population [SOURCE]. 

To build on the patterns we saw in figures seven and eight, we once again rely on the multivariate linear model(age replacing air travel) to find out the new correlation between pandemic preparedness and cumulative deaths per million when age acts as a confounding variable.

As shown above, with age acting as a confounding variable, the correlation between pandemic preparedness and cumulative deaths per million falls from 9.09 to 5.76. In addition, the lower range of uncertainty decreases from 6.43 to 2.13. This steep drop off in correlation and uncertainty suggests that age did play a very significant role in the high positive correlation that we initially observed.

Furthermore, when we add air travel back to the model while retaining age, we observe a decrease in correlation from the initial 9.09 to 3.68 with the lower range of uncertainty being -1.40. This supports both our previous claims that air travel and age played a role in the unanticipated initial plots that we obtained.
